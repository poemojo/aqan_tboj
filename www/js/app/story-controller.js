...
storyController.controller('MainController', ['$http', '$cookies', 'StoryScene', 'ExDate', function($http, $cookies, StoryScene, ExDate)
{
   ...
   ctrl.initialize = function()
   {
      ...
      setTimeout(function(){imgScroll = new IScroll('#sceneImage', {scrollX: true, scrollY: true});}, 100);
      setTimeout(function(){advScroll = new IScroll('#advancePanel', {scrollX: false, scrollY: true, scrollbars: true, interactiveScrollbars: true, fadeScrollbars: true});}, 100);
   ...
   ctrl.setScene = function(sceneSrc, m)
   {
      ...
      .error(function(err)
      {
         var filename = null,
            id = null,
            title = "Error!",
            body = ["There ain't no scene here"],
            image = null,
            items = [],
            children = [],
            msg = "",
